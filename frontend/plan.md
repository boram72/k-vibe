...
- [ ] **지도 — 찜/검색결과/관광지추천/SNS분석기 활성화 중 필터탭 숨김 + 찜/관광지추천 닫기버튼 (11, 신규)** (`"11\. 찜"` 검색) — 미착수, 설계 전.
...
- [x] **13. 위치확인 버튼을 토글로 변경** — 현재는 클릭 1회성 조회 버튼(`RouteLocationCheck`)이었는데, 토글 형태로 변경. 켜면 기존과 동일(GPS 조회+미니맵에 내 위치 핀), 다시 눌러서 끄면 내 위치 핀 제거하고 지도 중심을 루트 기준으로 되돌림.
  ...
  - **후속 버그 발견·수정 1(2026-09-18, 대화 중 요청)**: 스팟이 하나뿐이고 내 위치와의 거리가 먼 경우, 토글을 켰다가 끄면 지도가 스팟 위치로 안 돌아가는 문제 발견. **원인**: `checkDistance()`의 `getCurrentPosition`은 비동기(`enableHighAccuracy: true`라 최대 5초까지 걸릴 수 있음)인데, 콜백이 도착하기 전에 토글을 꺼도 이를 확인하는 가드가 없었음 — 늦게 도착한 GPS 성공 콜백이 `onLocationChecked(coords)`를 다시 호출해서 방금 끈 `currentLocation`을 되살렸음. **수정**: `enabledRef`(토글 on/off를 동기적으로 반영하는 ref)를 추가해, `getCurrentPosition`의 성공/실패 콜백 모두 시작 시 `enabledRef.current` 확인 후 꺼져있으면 그냥 무시하도록 가드 추가.
  - **후속 버그 발견·수정 2(2026-09-18, 1번 수정 후 재확인 중 발견)**: 1번을 고친 뒤에도 "스팟 근처로는 가는데 그 스팟이 정확히 뜨는 자리로는 안 감"(1~2km 오차) 재보고 — 실측 결과 실제로 재현됨. **원인**: `route-mini-map.tsx`의 `fitKakaoMapToRoute()`가 스팟 1개 케이스에서 `map.setCenter()`를 먼저 부르고 `map.setLevel(4)`를 나중에 불렀는데, 레벨을 큰 폭으로 바꿀 때(내 위치 포함 넓은 bounds였다가 스팟 하나로 좁혀지는 경우 레벨 14→4처럼 10단계 이상 점프) 카카오 SDK가 줌 기준점을 다시 잡으면서 center가 살짝 어긋남(직접 카카오 지도 인스턴스에 임시 디버그 훅을 걸어 `getCenter()`로 실측: 목표 좌표에서 약 1.3km 차이). **수정**: 마지막 호출이 항상 이기도록 `setLevel(4)`를 먼저, `setCenter()`를 나중에 부르는 순서로 변경. Playwright로 실제 5173 서버에 대해 (서울=현위치, 창원=단일 스팟) 시나리오 재현 — 수정 전 1.3km 오차 → 수정 후 0.001km(오차 사실상 0)로 확인.
...
- [x] **6. (완료) 지도 초기 랜딩·현재위치 동작 변경 — GPS를 서버로 보내지 않기 위한 우회 (2026-09-18, PR #116에 병합됨)**
  ...
  - **6-1. (완료) 초기 랜딩**: 지도 진입 시 사용자 위치가 속한 행정구역의 관할 정부처(예: 서울 → 서울특별시청, 창원 → 창원시청)로 랜딩. 지도 뷰와 서버 검색 좌표 둘 다 이 좌표를 그대로 씀(둘 다 6-2에서 만든 `setSearchCenter`+`setQueryCenter` 명시적 트리거를 그대로 재사용 — 신규 장치 불필요).
    ...
    - **버그 발견·수정 1**: GPS 요청(모킹 시 즉시 응답)이 카카오 SDK 스크립트 로딩(비동기, `MapCanvas` 내부의 `useKakaoLoader`가 관리)보다 먼저 끝나서 `resolveAdminOfficeCoords` 호출 시점에 `kakao.maps.services`가 아직 없어 항상 실패 → 서울 폴백만 계속 타던 문제 발견(디버그 로그로 직접 확인). `waitForKakaoServices()`(최대 5초, 100ms 간격 재시도)를 추가해 SDK가 뜰 때까지 기다린 뒤 진행하도록 수정.
    - **실패 시 폴백**: SDK 로드 실패/지오코딩 실패/정부처 검색 실패 등 어떤 이유로든 관할 정부처를 못 찾으면 **절대 실측 GPS로 폴백하지 않고** `SEOUL_CENTER`(서울시청과 거의 동일 좌표)로 디폴트 랜딩(대화로 확정).
    - `use-current-location.ts`의 `requestLocation()`을 `Promise<Coordinates>` 반환하도록 변경(신선한 GPS/마지막 위치 캐시/서울 폴백 중 무엇으로 정해지든 그 최종 좌표를 호출부가 이어받아 행정구역 판별에 쓸 수 있도록) — 기존 호출부(RadarPage 등)는 반환값을 안 써도 무방해 하위호환 유지.
    - **UX 개선(대화 중 요청)**: 랜딩이 확정되기 전까지 실측 GPS 위치가 잠깐 보였다가 정부처 좌표로 "점프"하는 게 버그처럼 보인다는 피드백 — `isResolvingLanding` state를 추가해 랜딩 확정 전까진 지도/목록 자리에 스켈레톤을 보여줌. **버그 발견·수정 2**: 처음엔 스켈레톤이 `MapCanvas` 자체를 통째로 안 그리는 방식으로 구현했는데, `useKakaoLoader()`가 `MapCanvas` 안에 있어서 그 컴포넌트가 마운트조차 안 되면 SDK 자체가 로드를 시작 못 해 매번 5초 타임아웃 후 서울로만 폴백되는 버그가 있었음(실사용 재현으로 발견) — `MapCanvas`는 항상 마운트해서 SDK가 백그라운드에서 계속 로드되게 하고, 스켈레톤은 그 위에 `absolute` 오버레이로 겹치는 방식으로 수정. 겹친 스켈레톤이 카카오 SDK의 내부 버튼(위치버튼/줌컨트롤 등, z-10~20)보다 아래에 깔려 실측 위치가 살짝 비쳐 보이던 것도 `z-30`으로 올려 해결.
    - **검증**: Playwright로 실제 5173+백엔드 대상 검증 — ① 창원 인근 GPS 모킹 → 서버 `/places` 쿼리와 지도 뷰 둘 다 정확히 창원시청 좌표(`35.2279, 128.6818`, REST API로 실측 확인한 실제 건물 좌표와 소수점까지 일치)로 착륙 ② 강남 인근 GPS 모킹 → 구청이 아니라 서울특별시청(`37.5668, 126.9787`)으로 착륙(광역시 기준 확인) ③ 카카오 지도 인스턴스에 임시 디버그 훅을 걸어 `getCenter()`로 지도 뷰 자체도 동일 좌표로 이동한 것 직접 확인 ④ 스켈레톤이 약 1.2초 만에 자연스럽게 사라지고 실측 위치 노출 없이 랜딩되는 것 확인.
    - **참고**: 이전 세션에서 만들었던 카카오 SDK "검색 버튼 로딩중" 표시 수정(`fix/map-search-loading-state` 브랜치, 커밋 `3b8e563`)이 PR 안 올라간 채로 남아있는 것 발견 — 이번 작업과는 별개로 나중에 마저 처리 필요.
  - **6-2. (완료) "현재위치" 버튼 자동 재검색 제거**: 지금까지는 "현재위치" 버튼을 누르면 자동으로 그 좌표 기준 서버 재검색(`/places` 재조회)까지 같이 일어났음.
    - **시행착오 1**: 처음엔 서버로 나가는 좌표(`queryCoords`)를 `searchCenter ?? initialQueryCoords`(GPS 최초 해석값을 얼려서 고정)로 뷰 좌표(`effectiveCoords` = `searchCenter ?? coords`)와 다른 폴백 규칙으로 파생시켰는데, 두 값이 `searchCenter`를 공유하면서 서로 다른 우선순위를 기대하다 보니 `handleRequestLocation()`이 `searchCenter`를 null로 되돌리는 순간(뷰가 GPS를 되찾게 하려고) `queryCoords`도 같이 얼려둔 값으로 스냅되어 버려 재검색이 발생 — 그래서 `setSearchCenter(null)` 자체를 지웠더니 이번엔 반대로 **뷰가 GPS로 안 움직이고 최근 검색 좌표에 고착되는 버그**가 생김(사용자가 실사용 중 재현·지적).
    - **최종 방향(사용자 제안, 채택)**: 뷰 좌표(`effectiveCoords`)는 원래 형태(`searchCenter ?? coords`)로 완전히 되돌리고, `handleRequestLocation()`도 원래대로 `setSearchCenter(null)`을 포함한 전체 초기화 로직을 되돌림. 대신 **서버로 나가는 좌표(`queryCenter`)를 `searchCenter`의 파생값이 아니라 완전히 독립된 state**로 분리해서, `setSearchCenter`를 호출하는 딱 4곳(주소 매칭 2곳 — 검색창 지역검색/관광지 클릭, 관광지 키워드 매칭 1곳, 드래그확인 "이 지역에서 검색" 1곳)에서만 `setQueryCenter`도 나란히 호출. `/places`·`RelatedAttractionsList`(`/attractions/related`)는 이 `queryCenter` 기반 `queryCoords`를 쓰고, `MapCanvas`(지도 뷰)는 그대로 `effectiveCoords`를 씀. 두 값이 서로 다른 폴백 규칙을 지닌 파생값이 아니라 각자 독립적으로 관리되므로, `searchCenter`를 null로 되돌려도 `queryCenter`는 전혀 영향받지 않음 — 뷰/검색이 서로의 우선순위 규칙과 부딪힐 일이 구조적으로 없어짐. `initialQueryCoords`/`firstCoordsRef`/`capturedInitialQueryRef` 같은 "얼려두기" 장치 전부 제거되어 코드도 더 짧아짐.
    - **검증**: Playwright로 실제 5173+백엔드 대상 검증 — ① 초기 로드 시 `/places` 1회 ② 지도 드래그 후 "이 지역에서 검색" 클릭 시 새 좌표로 정상 재호출 ③ 드래그+검색 결과를 얻은 뒤 "현재위치" 클릭 시 **추가 서버 호출 없이** 드래그검색 결과가 그대로 유지 ④ 카카오 지도 인스턴스에 임시 디버그 훅을 걸어 `getCenter()` 직접 확인 — 드래그 이동 후에도 "현재위치" 클릭 시 지도 뷰가 실제 GPS 좌표로 정확히 스냅백되는 것 확인(뷰 고착 버그 해소 재확인).
  - **검토 결과 — 이 우회가 실제로 유효한지**: 백엔드(`presentation_api/places.py`)를 확인한 결과 `/places`는 요청받은 lat/lng을 TourAPI 호출에 그때그때 쓰기만 하고, 그 값 자체를 유저와 연결해 DB에 저장하거나 로그로 남기지 않음(캐싱되는 건 검색 *결과* 장소 데이터뿐, 검색에 쓴 좌표가 아님) — 서버 좌표 자체를 유저 식별 정보와 결합해 영속 저장하는 경로는 없음을 확인. **말씀하신 "지도 center값(=행정구역 관할 정부처처럼 다수 사용자가 공유하는 고정된 공공 지점)을 보내는 건 특정 개인의 실측 위치가 아니라서 개인위치정보에 해당하지 않는다"는 방향은 기술적으로 구현 가능하고 합리적인 우회로 판단됨.**
  - **실제로 바뀌는 지점은 6-1, 6-2 이 둘뿐**(대화로 확정) — "현재위치" 버튼 클릭 자체는 서버 호출이 아예 없어지므로(6-2), 그 좌표를 따로 뭉뚱그릴 필요가 없음. 클릭하면 카카오 뷰만 실측 GPS로 이동(지도 위 "내 위치" 표시 등 화면 표시 용도는 실측 GPS 그대로 사용 — 서버로 안 나가면 문제 없음), 이후 사용자가 손으로 지도를 옮기고 "이 지역에서 검색"을 눌러야만 그 시점의 지도 center값이 서버로 나감 — 이건 기존에 이미 있던 드래그검색 흐름 그대로라 별도 처리 불필요.
  - **추가로 고려해야 할 사항**:
    1. **행정구역 판별 자체도 클라이언트에서만 처리** — GPS 실측 좌표 → "어느 시/군/구인지" 판별(reverse geocoding)을 우리 백엔드로 보내서 처리하면 우회 의미가 없어짐. **카카오 Geocoder를 브라우저에서 카카오 서버로 직접 호출**(동적 검색, 정적 좌표 테이블 하드코딩 안 함)하는 방식으로 확정 — 우리 서버는 실측 GPS를 끝까지 못 봄. "{지역명} 시청/군청/구청" 지오코딩도 동일하게 클라이언트에서 카카오 Geocoder로 처리.
    2. **뭉뚱그리는 단위(granularity)가 중요** — 시/군/구(관할 정부처) 수준까지만 좁히고, 그보다 더 정밀한 동/읍/면 단위나 GPS 그대로에 가까운 반올림은 지양. 정부처 좌표는 다수 사용자가 공유하는 고정된 공공 지점이라 "특정 개인의 위치"로 보기 어렵다는 논리가 성립하는데, 더 잘게 쪼갤수록 그 논리가 약해짐.
    3. **"마지막 위치" 폴백 캐시(`location-cache.ts`)도 랜딩 로직에서 동일하게 처리 필요** — GPS 권한 거부 시 쓰는 `readLastKnownLocation()` 폴백도 과거에 저장해둔 실측 GPS라, 6-1(초기 랜딩)의 "행정구역 판별 → 관할 정부처 좌표로 검색" 파이프라인을 신선한 GPS든 캐시 폴백이든 동일하게 거치도록 해야 함(신선한 GPS 경로만 처리하고 캐시 폴백 경로를 빠뜨리기 쉬움).
    4. **전수 조사 결과(2026-09-18)** — `useCurrentLocation()`/`navigator.geolocation`을 쓰는 곳 전체를 추적함.
       - **안전(그대로 둠)**: `route-location-check.tsx`(거리 계산+미니맵 표시용, 서버 호출 없음), `MapCanvas`의 `myLocation`(지도 위 "내 위치" 파란 점 표시 전용) — 이게 바로 "GPS는 지도 뷰에서만" 원칙에 맞는 정상 사례.
       - **6번 작업으로 자동 해결됨(테스트 시 확인 필요)**: `MapPage`의 "관광지 추천" 목록(`RelatedAttractionsList` → `GET /attractions/related`)도 `SpotListPanel`의 `center` prop, 즉 `effectiveCoords`를 그대로 받아쓰는 구조라 6-1/6-2 수정 시 코드 추가 변경 없이 같이 해결됨. 다만 실제로 관할 정부처 좌표로 나가는지 이 목록도 같이 테스트 필요.
       - **여전히 미해결(별도 결정 필요)**: **RadarPage(`/radar`)** — 내비게이션에서는 숨겨져 있지만 라우트/코드가 살아있어 URL로 직접 접근 가능하고, `useCurrentLocation()`의 실측 `coords`를 반경/필터 바뀔 때마다 드래그 확인 절차도 없이 그대로 `GET /amenities`에 보내는 구조(지도보다 더 무방비 — 자동 재요청). 이번 변경 범위(지도)와 별개 이슈이므로, 이 우회가 회사 정책상 앱 전체에 적용돼야 하는 요건이라면 RadarPage도 같이 손보거나 최소한 라우트 자체를 완전히 접근 차단할지 결정 필요.
    5. **인프라/호스팅 레벨 접근 로그** — 이번 검토는 애플리케이션 코드(백엔드 라우터/DB) 기준이고, 리버스 프록시나 호스팅 플랫폼이 요청 전체 URL(쿼리스트링 lat/lng 포함)을 IP와 함께 액세스 로그로 남기는지는 별도 확인 필요(코드베이스 밖의 인프라 설정이라 이 조사로는 확인 불가) — 로그를 남긴다면 보존기간·마스킹 정책도 같이 점검 필요.
    6. **법적 판단 최종 확인 필요** — 위 검토는 코드 동작 기준의 기술적 판단이고, "관할 정부처 좌표 스냅"이 위치정보법상 실제로 등록 의무를 회피하기에 충분한지는 법률 검토(사내 법무팀 또는 관련 전문가 확인)를 별도로 받는 걸 권장.
  - **구현 방식 확정(대화로 확정)**: 정부처 좌표는 정적 데이터 하드코딩 없이 **카카오 Geocoder 동적 검색**으로, 행정구역 판별도 **프론트에서만** 처리. 결정 완료 — 실제 구현 시작 가능.
...
## ⬜ 2026-09 장소 리뷰 수정/삭제 기능 추가 (대화 중 요청, 담당자: 보람)
...
- [x] **1. (완료) 내 루트 subtitle 정리** — `RoutePage.tsx`의 "내 루트" 제목 아래 안내문(`route.helper`: "방문지를 드래그해 순서를 바꾸세요. 시각을 탭하면 직접 설정할 수 있어요.") 최초 삭제. Step14 후속에서 시간 계산 UI(시각 편집 다이얼로그 등)를 이미 통째로 제거했는데 이 안내문만 안 지워져서 존재하지 않는 기능("시각을 탭하면...")을 안내하는 상태였음.
  - **후속(대화 중 요청)**: 완전히 없애니 허전하다는 피드백 — 지금 실제로 있는 기능 기준으로 새 문구("완료 여부와 총 이동거리를 한눈에 확인할 수 있어요" 추가)로 교체했었으나, 이후 `origin/main` 병합 과정에서 **팀원(hslee)이 같은 문제를 이미 독립적으로 고쳐둔 것**(`route.helper`를 "방문지를 드래그해 순서를 바꾸세요."로 트리밍만 하고 문장 추가는 안 함)을 발견 — 최종적으로 **팀원 버전을 그대로 채택**(대화로 확정). 4개 언어 전부 팀원의 짧은 문구로 되돌림.
...
- [x] **4-1. (완료 — 의도된 동작으로 확정, 코드 수정 없음) 여러 장소 bounds-fit 시 검색 힌트가 그 위치를 못 따라감** — 4번 구현 후 전수 감사 중 발견. `map-canvas.tsx`의 `fitKakaoMapToPlaces()`가 장소 2개 이상을 한 화면에 담을 때(페르소나/스타 필터로 여러 장소 선택, 또는 SNS 분석기에서 2개 이상 핸드오프) `map.setBounds(...)`를 카카오 지도 인스턴스에 직접(imperatively) 호출해서, 4번의 `realCameraCenter` 추적(`center`/`focusCenter`/`pendingCenter` 기반)이 이 경우엔 갱신되지 않는 문제로 보고됐었음.
  - **결론(대화로 확정)**: bounds-fit은 줌아웃해서 여러 장소를 한 화면에 담는 동작이라, "지금 화면의 기준 좌표 1개"를 애초에 정의하기 어려움(예: 서울+부산에 흩어진 스팟을 다 담았을 때 그 중 어디를 검색 기준점으로 삼아야 하는지 자체가 불명확). 이런 경우 검색 힌트를 갱신하지 않고 **bounds-fit 직전의 마지막 위치를 그대로 유지**하는 지금 동작이 오히려 올바른 선택 — 버그 아님, 수정 불필요로 종료.

- [ ] **5. 주변스팟 영역 처리 필요 (2026-09-19, 대화 중 요청, 신규)** — 우측 목록 패널에 섹션이 여러 개(검색결과/찜/이 지역 연관 관광지/주변 스팟) 켜질 수 있는데, 각 섹션이 차지하는 영역 때문에 정작 "주변 스팟"이 화면에 아예 안 보이는 문제. 찜/연관관광지 섹션 자체도 영역이 매우 협소함.
  - **5-1. (완료)** SNS 분석기에서 지도로 아이템을 불러온 경우(`focusPlaces` 핸드오프), 예전엔 그 아이템들이 "주변 스팟" 섹션에 그냥 섞여서 나열됐음(타이틀만 "SNS 분석기"로 바뀌고 목록 자체는 안 나뉘어 있었음) — 이후 대화 중 지적받아 **완전히 독립된 섹션으로 분리**함: `spot-list-panel.tsx`에 `analyzerPlaces` prop 신규(검색결과 섹션보다도 위, 최우선 노출) + `MapPage.tsx`가 `nearbySpotListPlaces = filtered.filter(주변 스팟에서 focusPlaces 항목 제외)`를 계산해서 "주변 스팟" 목록에는 더 이상 안 섞이게 함. 지도 핀은 영향 없음(핀은 여전히 `filtered` 전체 — focusPlaces 핀도 그대로 보임, 목록 패널 표시만 분리). Playwright로 "SNS Analyzer" 섹션(5개)/"Nearby Spots" 섹션(30개)이 서로 겹치지 않고 각자 뜨는 것 확인.
  - **5-2. (완료)** 검색결과/찜/이 지역 연관 관광지/SNS 분석기가 다 켜지면 각 섹션이 차지하는 영역 때문에 주변 스팟이 전혀 안 보임(찜/연관관광지 영역도 매우 협소) — Playwright로 재현: 3개 섹션 동시 노출 시 "Nearby Spots" 타이틀이 뷰포트(844px) 밖(y=953)까지 밀려나 완전히 안 보임.
    - **구현**: 검색결과/찜/연관관광지/SNS분석기/주변 스팟(또는 페르소나 방문 장소)을 독립된 boolean 여러 개로 각자 켜고 끄는 대신, `MapPage.tsx`에 `activeSection: 'saved' | 'attractions' | 'searchResults' | 'analyzer' | null`(모듈 스코프 `ActiveSection` 타입으로 export, `SpotListPanel`이 import) 단일 상태로 관리 — `null`이면 기본값인 주변 스팟/페르소나 방문 장소. `spot-list-panel.tsx`는 각 섹션을 "고정 타이틀(닫기/카운트 포함)"과 "그 아래 남는 공간을 전부 채우는 스크롤 목록" 한 쌍(`analyzerTitle`/`analyzerList`, `areaSearchResultsTitle`/`areaSearchResultsList`, `savedTitle`/`savedList`, 기본 `titleRow`/`listRegion`)으로 만들어두고, `activeSection`에 해당하는 딱 한 쌍만 렌더링 — **"하나의 이벤트엔 하나의 목록만" 원칙(대화로 확정)**: 다른 섹션이 활성화되면 "주변 스팟"도 예외 없이 완전히 숨겨짐(예전처럼 위에 쌓이기만 하는 게 아니라 아예 안 그려짐). 연관 관광지(`RelatedAttractionsList`)는 컴포넌트 내부에 타이틀이 박혀있어 이번 범위에선 안 건드림(기존처럼 타이틀도 같이 스크롤).
    - **우선순위 원칙 — "방금 한 행동이 우선"(recency)**: 찜 버튼을 누르면 `activeSection`이 `'saved'`로 전환되고 다른 섹션은 전부 자동으로 닫힘. 이어서 검색하면 `'searchResults'`로 전환(찜 자동으로 닫힘). 검색결과를 닫으면(5-4의 X 버튼) `null`로 복귀. 이미 켜진 토글을 다시 누르면(예: 찜 상태에서 찜 버튼 재클릭) 마찬가지로 `null` 복귀.
    - **찜/연관관광지 토글도 상호배타로 변경**: 예전엔 두 토글 버튼(하트/스파클)이 각자 독립 boolean이라 동시에 둘 다 켜는 중복 클릭이 가능했음 — `activeSection` 하나로 흡수해서 하나를 켜면 다른 하나는 자동으로 꺼짐(동시 노출 자체가 불가능).
    - **SNS 분석기 섹션도 검색결과처럼 닫기 버튼 추가**: `analyzerSection`에 X 버튼(`onCloseAnalyzerSection`) 추가.
    - **"내 루트"에서 온 핸드오프는 별도 activeSection이 아님(대화로 확정)** — `MapFocusState.returnToRoute`(내 루트 스팟 카드에서 지도 아이콘 클릭, 항상 `focusPlaces.length === 1`)로 들어온 경우는 자기만의 섹션 없이 평소처럼 "주변 스팟" 목록에 포함되고, 그 장소 행에만 "My Route" 태그가 붙음(`routeOriginPlaceId`를 `SpotListPanel`까지 내려서 `renderPlaceRow`에서 비교). `activeSection` 초기값은 `focusState?.returnToRoute`일 땐 `'analyzer'`가 아니라 `null`.
    - **구현 중 대화로 추가 확정된 원칙 — "지도 핀은 항상 지금 보이는 목록과 일치"**: 처음엔 "SNS 분석기 닫기 = 목록 섹션만 바뀌고 지도 핀/카메라는 그대로"로 구현했으나, 대화 중 "닫았는데 계속 분석기 장소가 지도에 보인다"는 지적을 받고 원칙을 재정의 — **지도에 표시되는 핀은 예외 없이 "지금 화면에 보이는 목록에 있는 장소들"과 정확히 일치**해야 함(찜/연관관광지도 동일). `MapPage.tsx`에 `mapPlaces`(activeSection별로 focusPlaces/areaSearchListPlaces/savedPlaces/`[]`(연관관광지, 좌표 없음)/nearbySpotListPlaces 중 하나를 고름)를 신설해 `<MapCanvas places>`로 교체 — SNS 분석기를 닫으면 그 장소들이 지도에서도 사라지되(빨간 핀도 같이 사라짐), 그중 "주변 스팟"에도 포함되는 장소(반경 내 실제 장소)는 activeSection이 `null`로 바뀌며 `nearbySpotListPlaces`로 자연 전환되는 과정에서 계속 남음 — 별도 예외처리 불필요. **지도 뷰(카메라)는 안 움직임** — SNS 분석기가 활성화된 동안엔 `mapHighlightIds`가 `focusPlaces` 전체를 빨간 핀으로 강조하고, 닫으면 강조만 사라짐(`effectiveCoords`/카메라 위치는 무관하게 유지).
    - **검증**: Playwright로 ① 찜→연관관광지→검색→닫기→찜(재토글) 전 과정에서 매 순간 타이틀이 정확히 1개만 존재함을 확인(`main p.text-sm.font-bold` 텍스트 배열로 검증), ② SNS 분석기 핸드오프 도착 시 빨간 핀 5개 확인, 닫기 클릭 후 빨간 핀 0개로 확인, 상단 라벨("Analysis Result")이 닫기 전후 동일해 카메라가 안 움직였음을 확인, ③ `returnToRoute` 핸드오프 시 타이틀이 "SNS Analyzer"가 아니라 "Nearby Spots"로 뜨고 해당 행에 "My Route" 태그가 정확히 붙는 것을 DOM에서 직접 확인. `npx tsc --noEmit -p tsconfig.app.json`(캐시 삭제 후)/`npx eslint src` 클린(shadcn 3건 제외).
  - **5-3.** 타이틀 변경 부분 **(완료)** — 페르소나별(스타별) 탭에서 "주변 스팟" 대신 "페르소나 방문 장소"로 타이틀 이름 변경(`MapPage.tsx`의 `spotListTitle`).
    - **버그 원인 조사 완료(수정 안 함, 대화 중 요청)** — 대화로 지적받은 2개 증상을 Playwright로 재현·검증:
      1. **"페르소나 클릭 시 줌아웃 대신 그냥 다른 도시로 이동"**: 창원시청(GPS)에서 페르소나별 탭 진입 시 지도가 서울로 바로 점프함(줌아웃해서 창원+서울을 같이 담는 게 아님). **원인**: `map-canvas.tsx`의 `fitKakaoMapToPlaces()`가 `map.setBounds()`를 계산할 때 **선택된 페르소나의 스팟 좌표만** 사용하고 사용자의 실제 현재 위치(GPS)는 bounds 계산에 전혀 포함하지 않음 — 그래서 "여러 지점을 다 담기 위한 줌아웃"이 아니라 "페르소나 스팟들이 있는 곳으로 그냥 이동"처럼 보임. Playwright로 재현: 창원(GPS) → "By Persona" 탭 진입만 해도 지도가 즉시 서울(도산공원 일대)로 이동, IU 선택 시에도 동일(서울시청 인근으로 이동).
      2. **"페르소나 필터 적용 중엔 장소 검색/이 지역 검색이 안 먹힘"**: 실제로는 지도 카메라 이동과 `/places` 재조회 네트워크 요청 자체는 정상 발생함(Playwright로 확인 — 드래그 후 "이 지역에서 검색" 클릭 시 지도가 실제로 새 위치(강남 일대)로 이동하고 `/places?lat=...&lng=...`(강남 좌표) 요청도 정상 발생). **하지만 "페르소나 방문 장소" 목록 자체는 검색/이동 전후로 개수·내용이 완전히 동일**(예: IU 선택 시 5개 → 검색 후에도 그대로 5개, 여전히 도산공원/이화벽화마을 등 원래 5곳). **원인**: 카테고리별 탭은 위치 기반으로 받아온 `/places` 결과를 카테고리 속성으로만 걸러내는 "순수 필터"인 반면, 페르소나별 탭은 `MapPage.tsx`의 `filtered`(`matchStar` 조건)가 위치와 무관한 **고정된 전역 카탈로그**(`personaPlaces`, 특정 스타의 정해진 방문지 4~5곳)를 태그 일치 여부로만 골라 보여줌 — 지도를 어디로 옮기든, `/places`를 다시 받아오든 이 목록의 소스 자체가 그 결과를 안 씀. 즉 "페르소나 필터는 카테고리 필터와 기능상 동일해야 한다"는 전제와 달리, 페르소나별 탭은 필터가 아니라 **데이터 소스 자체를 통째로 바꾸는 스위치**에 가까움 — 이게 "검색/이동이 안 먹히는 것처럼 보이는" 근본 원인.
      - **1번(줌 점프)은 아래 6/7번과 함께 완료** — 상세는 6/7번 항목 참고. **2번(위치 무관 고정 카탈로그)은 이번 라운드에서 수정 안 함** — 필요 시 향후 페르소나별 탭의 목록도 위치 기반 `/places` 결과와 결합하는 방향으로 재설계 검토.
  - **5-4. (완료)** 검색결과 목록 닫기 버튼 추가 — `spot-list-panel.tsx`의 `areaSearchResultsSection`에 X 버튼(`onCloseAreaSearchResults` → `MapPage`에서 `setAreaSearchLists(null)`) 추가. 검색창 텍스트/`areaSearchedQuery`는 안 건드림(뭘 검색했었는지 계속 보이게 하는 기존 동작과는 별개). 이 작업 중 **타이틀 레이어가 목록과 같이 스크롤되던 버그도 같이 발견·수정**: `areaSearchResultsSection`의 타이틀+닫기버튼이 원래 `overflow-y-auto` 스크롤 컨테이너 안에 있어서 목록을 스크롤하면 닫기 버튼까지 같이 밀려 올라가 사라졌음 — "주변 스팟" 타이틀(`titleRow`)/찜 목록(`savedListSection`)처럼 타이틀은 스크롤 밖으로 빼고 목록만 스크롤되게 통일. Playwright로 스크롤 전/후 닫기 버튼 좌표가 완전히 동일함을 확인.

- [x] **6. (완료) 모바일 — 이 지역 연관 관광지 등 특정 스팟 검색/클릭 시 줌인 안 됨 (2026-09-19, 대화 중 요청)**
  - **원인(Playwright 실측 확인)**: `<KakaoMap level={4}>`의 `level`이 매 렌더마다 항상 같은 리터럴 값 `4`라서, 마운트 시 한 번 적용된 뒤로는 react-kakao-maps-sdk가 값이 "안 바뀌었다"고 보고 `setLevel()`을 다시 안 부름. 반면 여러 장소를 담는 `fitKakaoMapToPlaces()`(bounds-fit)는 `map.setBounds()`로 실제 줌 레벨을 React 밖에서 직접 바꿔버림. 이후 특정 장소 하나를 선택(focusCenter 경로)하면 카메라 위치(center)는 정확히 그 장소로 이동하지만, 줌 레벨은 이전 bounds-fit이 남긴 값 그대로 남아 "줌인이 안 된 것"처럼 보임. 실측: 페르소나 "전체" 진입(level 9로 줌아웃) → 특정 스팟 클릭 → center는 정확히 이동, **level은 계속 9**.
  - **구현**: `map-canvas.tsx`의 `KakaoMapCanvas`에 `focusCenter`가 세팅될 때(장소 하나를 포커스할 때) `map.setLevel(4)`를 명시적으로 호출하는 `useEffect` 추가. **구현 중 추가 발견**: `setLevel()`만 부르면 `<KakaoMap center>` prop 변경이 트리거한 `isPanto` 애니메이션이 중간에 끊겨서 줌만 4로 바뀌고 카메라는 bounds-fit 위치에 그대로 멈춰버림 — `map.panTo(focusCenter)`도 같이 명시적으로 호출해 두 상태를 한 번에 확정하도록 수정(`handleRequestLocation`과 동일한 방어 패턴).
  - **검증**: Playwright(임시 `window.__debugMap` 노출 후 검증 종료 시 원복)로 페르소나 "전체" 진입(level 9) → 특정 스팟("도산공원") 클릭 → level 4 + center가 정확히 도산공원 좌표(37.524/127.035)로 이동함을 실측 확인. `npx tsc --noEmit -p tsconfig.app.json`/`npx eslint src` 클린.
- [x] **7. (완료) 모바일 — 특정 스팟 클릭 후 페르소나별 필터 클릭 시 한반도 전체로 줌아웃 (2026-09-19, 대화 중 요청)**
  - **원인**: 5-3 finding #1과 동일 계열 — `fitKakaoMapToPlaces()`가 현재 카메라 위치는 무시하고 `fitPlaces` 좌표만으로 매번 bounds를 다시 계산 + `boundsKey`가 바뀌면(카테고리별→페르소나별 전환 등) `focusCenter`가 무조건 null로 리셋되며 bounds-fit이 재실행됨. 실측: 특정 스팟 클릭(level 4) → 페르소나별(전체) 클릭 → level 9로 급격히 전환.
  - **⚠️ 방향 수정(대화 중 확정)**: 처음엔 "페르소나별 '전체' 진입 시 자동 카메라 이동을 아예 안 함"으로 계획했으나, **사용자가 "페르소나별 누르면 전체를 자동 선택한 걸로 보고 전체 목록이 보이도록 줌아웃되면 좋겠다"고 확인** — 즉 줌아웃 자체는 의도된 동작으로 유지, "느닷없는 점프처럼 보인다"는 체감만 개선하는 방향으로 변경.
  - **구현**: `map-canvas.tsx`의 `fitKakaoMapToPlaces(map, places, includeCoord?)`에 선택적 `includeCoord` 인자 추가 — 있으면 bounds 계산에 그 좌표도 포함(장소가 1개뿐이어도 bounds 경로 사용). `MapCanvasProps`에 `includeCameraInFit?: boolean` 신규, `KakaoMapCanvas`의 bounds-fit effect가 `includeCameraInFit`이 true일 때만 `center`를 같이 넘김. `MapPage.tsx`는 `fitPlacesIncludeCamera = !focusPlaces.length && personaFocusPlaces.length > 0`(페르소나별 bounds-fit일 때만 true, SNS 분석기 핸드오프 땐 false)를 계산해 전달 — SNS 분석기 쪽은 기존처럼 GPS 안 섞고 분석된 스팟만 기준으로 동작 유지.
  - **검증**: Playwright로 창원(GPS) → 페르소나별(전체) 진입 → center가 창원(35.23/128.68)과 페르소나 카탈로그(서울 일대) 사이 지점(36.41/127.78, level 13)으로 이동함을 실측 확인(수정 전엔 서울로 그대로 점프). 회귀 확인: 같은 창원 GPS로 SNS 분석기 핸드오프 시 center가 여전히 서울 스팟들 기준(37.58/126.97)으로만 계산되고 창원이 안 섞임을 확인. `npx tsc --noEmit -p tsconfig.app.json`/`npx eslint src` 클린.
  - **수정 방향(컨펌 완료)**: 페르소나별 진입/전환 시 bounds-fit(줌아웃) 자체는 그대로 실행하되, bounds 계산에 **사용자의 현재 카메라 위치도 같이 포함**시킴 — "그쪽으로 점프"가 아니라 "지금 위치 포함해서 확대"가 되도록. SNS 분석기 쪽 bounds-fit(focusPlaces 전용)은 이 변경과 무관하게 그대로 둠(GPS 섞으면 오히려 "분석된 곳으로 바로 보여주기" 목적과 어긋남).
  - **예상 동작**: 창원에서 페르소나별(전체) 클릭 → 창원 포함해서 전체 카탈로그를 담는 범위로 자연스럽게 확대(여전히 넓은 범위지만 "점프"가 아니라 "확대"로 느껴짐). 특정 스팟을 먼저 클릭한 뒤 페르소나별을 눌러도 마찬가지로 확대됨 — **이 시퀀스는 더 이상 버그가 아니라 의도된 동작**("전체 목록 보여주기"가 목적이므로).

- [x] **8. (완료) 페르소나별 "전체" 모드에서 카테고리별 주변 스팟이 안 사라짐 (2026-09-19, 대화 중 발견)**
  - **원인(Playwright 실측 확인 + 소스 대조)**: `personaPlaces`(진짜 페르소나 카탈로그, `src/api/personas.ts`의 `fetchPersonaPlaces()`)는 `tags: [persona.label]`처럼 **스타 이름 자체**를 태그로 씀. 반면 일반 `/places`(카테고리별 주변 스팟, `src/api/places.ts`의 `SEOUL_PLACES` 등)는 `tags: ['한복', '궁궐', '역사']`처럼 **페르소나와 무관한 설명용 태그**를 가짐. `MapPage.tsx`의 `filtered`(`matchStar`) 로직이 "전체"(특정 스타 미선택) 상태일 때 `place.tags?.length > 0`(태그가 하나라도 있으면 통과)로만 판단해서, 페르소나 라벨이 아닌 일반 설명 태그를 가진 카테고리별 장소도 전부 "페르소나 장소"로 착각해 계속 지도/목록에 남음. 실측: 카테고리 모드에서 보이던 8개 장소(경복궁/남산타워 등)가 페르소나(전체) 모드에서도 그대로 다 남아있음 확인.
  - **구현**: `MapPage.tsx`에 `personaLabels = useMemo(() => new Set(personaPlaces.flatMap(p => p.tags ?? [])), [personaPlaces])` 추가, `matchStar`의 "전체" 분기를 `place.tags?.length > 0` 대신 `place.tags?.some((tag) => personaLabels.has(tag))`로 교체(API 재호출 없음, 이미 불러와져 있는 `personaPlaces`에서만 계산).
  - **검증 중 발견 — 지금 실제 백엔드로는 이 수정 자체가 눈에 안 보임**: 라이브 백엔드가 카테고리 장소에 `tags: []`(빈 배열)를 내려줘서, 수정 전에도 이미 "태그 없음"으로 안 보이고 있었음(버그가 우연히 안 재현되는 상태였음 — 8번 최초 조사 때도 이미 확인된 내용). 검증하려면 mock 폴백 경로가 필요 — Vite dev 서버가 `/backend` 프록시로 기본 폴백하는 구조(`src/api/client.ts`)라 `VITE_API_BASE_URL`만 비우는 걸로는 mock이 강제 안 됨(dev 모드에서 `VITE_API_BASE_URL || '/backend'`로 항상 프록시를 타기 때문) — 백엔드 프로세스를 실제로 잠깐 내려서 프록시 호출이 502로 실패하게 만들어야 진짜 mock 폴백(`withFallback`)이 확인됨.
  - **검증**: 위 방법으로 mock 강제 후 Playwright로 확인 — 카테고리 모드 목록 8개(을지로 노가리골목/경복궁/북촌 한옥마을 스테이/광장시장 마약김밥/남산타워 포토스팟/홍대 놀이터/한강공원 여의도/성수동 카페거리) 중 "경복궁"을 제외한 나머지 7개는 페르소나(전체) 목록에 전혀 안 나타남 확인. "경복궁"만 페르소나(전체) 목록에도 보이는데, 이건 카테고리 mock의 경복궁(`id: '1'`, 거리 "1.4km" 표시)과는 별개로 BTS 뷔의 방문지 카탈로그에 실제로 존재하는 항목(`id: 'BTS뷔-경복궁'`, 정상 페르소나 태그 보유, 거리 표시 없음)이라 이름만 우연히 겹치는 정상 동작 — 진짜 leak 아님. `npx tsc --noEmit -p tsconfig.app.json`/`npx eslint src` 클린.

- [x] **9. (완료) 페르소나 탭에서 "이 지역에서 검색" 시 카테고리별로 자동 전환 안 됨 (2026-09-19, 대화 중 발견)**
  - **증상**: 페르소나별 탭에서 특정 장소를 클릭해 줌인된 상태 → 드래그 후 "이 지역에서 검색"(`onSearchArea`, `SearchAreaButton`)을 누르면, `center`가 바뀌며 `focusCenter`가 리셋되고 `boundsKey`(페르소나 카탈로그, 안 바뀜)로 인해 bounds-fit이 재실행되어 다시 전체 페르소나 목록으로 줌아웃됨 — 정작 사용자가 원한 "이 지역 주변 검색 결과"는 페르소나 카탈로그에 파묻혀 안 보임.
  - **범위 확정(대화로 확인)**: 드래그+확정 방식의 "이 지역에서 검색"(`onSearchArea`) **1건만** 대상 — 검색창 타이핑(엔터 안 침)은 페르소나 목록 내 텍스트 필터링이 맞는 동작이라 변경 안 함. 엔터를 쳐서 확정하는 순간은 5-2의 `activeSection='searchResults'`가 최상위로 뜨므로 이 문제 자체가 없음(수정 불필요).
  - **구현**: `MapPage.tsx`의 `onSearchArea` 콜백 안에서 `setSearchCenter`/`setQueryCenter`와 함께 `setFilterMode('category')`도 호출.
  - **검증**: Playwright로 페르소나별 탭 진입 → 지도 드래그 → "Search this area" 확정 → 탭이 "By Category"로 자동 전환되고 "Nearby Spots" 목록이 뜨는 것을 확인. `npx tsc --noEmit -p tsconfig.app.json`/`npx eslint src` 클린.
  - **예상 동작 확인됨**: `starFilter`(예: IU 선택 상태)는 안 지워지고 기억되어, 나중에 페르소나별로 다시 돌아가면 그대로 복원됨(기존 "탭 전환 시 서로 선택값 유지" 원칙과 동일).

- [x] **10. (완료) 페르소나 탭에서 같은 장소 재선택 시 줌인 안 됨 (2026-09-19, 대화 중 발견)**
  - **재현 시나리오**: 페르소나별 탭에서 장소 A 선택(줌인, 6번 수정으로 정상 동작) → 카테고리별 탭 이동 → 페르소나별로 복귀(boundsKey 변경으로 focusCenter가 null 리셋 → 7번 수정으로 정상 줌아웃) → 이 상태에서 **장소 A를 다시 클릭해도 줌인이 안 됨**.
  - **원인**: `MapPage.tsx`의 `highlightedPlaceId`가 이미 A로 설정된 채 바뀐 적이 없어서(카테고리↔페르소나 전환 동안 `selectedPlaceId` 자체는 계속 A로 동일), A를 다시 클릭해도 React가 "같은 값으로 set"이라고 판단해 리렌더를 안 일으킴 → `map-canvas.tsx`의 `selectedPlaceId !== prevSelectedPlaceId` 비교가 "변화 없음"으로 판단해 `focusCenter`를 다시 세팅 안 함.
  - **검토했다가 기각한 방법**: "boundsKey 리셋 시 selectedPlaceId가 유효하면 focusCenter를 그걸로 복원" — 이러면 페르소나 탭 전환 자체가 즉시 이전 선택 장소로 줌인되어버려서 7번에서 막 고친 "탭 전환 시 항상 전체 줌아웃" 동작이 깨짐(사이드 이펙트 확인, 기각).
  - **구현**: `MapPage.tsx`에 `selectionSeq` state 신규 추가(`handleSelectPlace` 호출마다 같은 장소여도 무조건 +1), `MapCanvasProps`에 `selectionSeq?: number` 추가해 `map-canvas.tsx`로 전달. `KakaoMapCanvas`의 변경 감지 조건을 `selectedPlaceId !== prevSelectedPlaceId || selectionSeq !== prevSelectionSeq`로 확장 — id가 같아도 seq가 바뀌면 focusCenter를 다시 세팅.
  - **부하/스케일 우려 확인 결과(대화 중 질문에 답변)**: `selectionSeq`는 브라우저 탭 하나의 React 메모리에만 존재 — 서버 전송/DB 저장 없음, 페이지 이탈/새로고침 시 자동 소멸(재마운트 시 0부터 재시작이라 별도 리셋 로직 불필요). 매번 숫자 하나만 교체(+1)되는 구조라 클릭 횟수와 무관하게 항상 O(1), 사용자 수와도 무관(각자 독립된 로컬 상태). 이 코드베이스에 이미 있던 `home-feed.tsx`의 `{category, nonce}` 패턴과 동일 원리.
  - **검증**: Playwright(임시 `window.__debugMap` 노출 후 원복)로 장소 A 선택(level 4)→카테고리별→페르소나별 복귀(level 8로 재줌아웃)→A 재클릭→**level 4 + 정확한 A 좌표로 복귀** 확인. `npx tsc --noEmit -p tsconfig.app.json`/`npx eslint src` 클린.

- [ ] **11. 찜/검색결과/이 지역 관광지 추천/SNS 분석기 활성화 중엔 카테고리·페르소나 필터 탭 숨김 + 찜/관광지 추천에도 닫기 버튼 (2026-09-19, 대화 중 요청) — 미착수**
  - **요청 내용**: `activeSection`이 찜/검색결과/이 지역 관광지 추천/SNS분석기 중 하나일 때는(5-2로 이미 상호배타 처리됨) 카테고리별/페르소나별 필터 탭(`filterTabs`)을 숨김 — 그 상태에서는 필터를 눌러도 각 섹션 자체가 자기만의 고정 데이터를 보여줄 뿐 필터에 반응하지 않으므로("어차피 필터 안 먹음") 탭을 계속 보여주는 게 오히려 혼란을 줌.
  - **요청 내용 2**: 지금은 검색결과/SNS분석기 섹션에만 닫기(X) 버튼이 있고, 찜/이 지역 관광지 추천은 기존 토글 버튼(하트/스파클 아이콘)을 다시 눌러야 꺼짐 — 사용자가 그 토글 재클릭 동작을 모를 수 있으니 대비 차원에서 찜/이 지역 관광지 추천 섹션에도 검색결과와 동일한 명시적 닫기(X) 버튼을 추가.
  - **다음 단계**: 아직 구현 방향 상세 설계 전 — 순차적으로 착수 예정.
