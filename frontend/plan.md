...
- [ ] **6. 지도 초기 랜딩·현재위치 동작 변경 — GPS를 서버로 보내지 않기 위한 우회 (2026-09-18, 대화 중 요청, 아직 미착수)**
  ...
  - **6-1. 초기 랜딩**: 지도 진입 시 사용자 위치가 속한 행정구역의 관할 정부처(예: 서울 → 서울시청, 창원 → 창원시청)로 랜딩. **지도 뷰(카카오 지도가 실제로 보여주는 중심좌표)와 서버 검색 좌표 둘 다 이 관할 정부처 좌표를 그대로 씀** — 검색만 이 좌표로 하고 지도 뷰는 실측 GPS로 보여주는 식으로 어긋나면 안 됨. 즉 `effectiveCoords`(지도 `center` prop이자 `/places` 쿼리 좌표)를 랜딩 시점부터 실측 GPS 대신 이 관할 정부처 좌표로 채움.
  - **6-2. "현재위치" 버튼 자동 재검색 제거**: 지금은 "현재위치" 버튼을 누르면 자동으로 그 좌표 기준 서버 재검색(`/places` 재조회)까지 같이 일어남 — 이걸 제거. 버튼을 누르면 지도만 그 위치로 이동하고, 이후 **지도를 손으로 움직여야만** "이 지역에서 검색" 버튼이 노출되고, 그 버튼을 눌러야만 서버 재검색이 일어남(기존 드래그 검색 흐름과 동일하게 통일).
