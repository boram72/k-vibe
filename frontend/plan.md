...
- [ ] **지도 — 페르소나별 탭 검색/줌아웃 버그 (5-3)** (`"5-3\."` 검색) — 원인 파악 완료(수정 안 함, 대화 중 요청): ① 페르소나 클릭 시 사용자 GPS는 무시하고 페르소나 스팟 좌표만으로 bounds를 잡아서 "줌아웃" 대신 "다른 도시로 점프"처럼 보임 ② 페르소나별 목록이 위치 무관 고정 카탈로그라 검색/이동해도 목록이 안 바뀜(카메라·`/places` 재조회 자체는 정상). 수정 방향 미정.
- [ ] **지도 — 모바일 특정 스팟 줌인 안 됨 (6)** (`"6\. 모바일 — 이 지역"` 검색) — 특정 스팟 검색/클릭 시 줌인이 안 됨. 원인 조사 전, 미착수.
- [ ] **지도 — 모바일 특정 스팟 클릭 후 페르소나 필터 시 한반도 전체 줌아웃 (7)** (`"7\. 모바일 — 특정 스팟"` 검색) — 5-3과 연관 가능성 있음(전국 흩어진 페르소나 스팟 bounds-fit). 원인 조사 전, 미착수.
...
- [ ] **5. 주변스팟 영역 처리 필요 (2026-09-19, 대화 중 요청, 신규)** — 우측 목록 패널에 섹션이 여러 개(검색결과/찜/이 지역 연관 관광지/주변 스팟) 켜질 수 있는데, 각 섹션이 차지하는 영역 때문에 정작 "주변 스팟"이 화면에 아예 안 보이는 문제. 찜/연관관광지 섹션 자체도 영역이 매우 협소함.
  ...
  - **5-3.** 타이틀 변경 부분 **(완료)** — 페르소나별(스타별) 탭에서 "주변 스팟" 대신 "페르소나 방문 장소"로 타이틀 이름 변경(`MapPage.tsx`의 `spotListTitle`).
    - **버그 원인 조사 완료(수정 안 함, 대화 중 요청)** — 대화로 지적받은 2개 증상을 Playwright로 재현·검증:
      ...
      - **수정하지 않음(사용자 지시) — 원인만 기록.** 필요 시 향후: (a) bounds 계산에 사용자 현재 위치도 같이 포함하거나, (b) 페르소나별 탭의 목록도 위치 기반 `/places` 결과와 결합하는 방향으로 재설계 검토.
...
- [ ] **6. 모바일 — 이 지역 연관 관광지 등 특정 스팟 검색/클릭 시 줌인 안 됨 (2026-09-19, 대화 중 요청, 신규, 미착수)** — 모바일에서 "이 지역 연관 관광지 추천" 등의 목록에서 특정 스팟을 검색하거나 클릭해도 지도가 그 스팟으로 줌인되지 않음. 원인 조사 전이라 확정 아님 — `map-canvas.tsx`의 장소 선택 처리(`selectedPlaceId` 변경 시 `focusCenter`만 세팅)는 카메라를 그 위치로 팬만 시키고 `map.setLevel()` 등으로 줌 레벨을 조정하는 코드가 없어 보이는데, 이게 관련 있는지 확인 필요.
- [ ] **7. 모바일 — 특정 스팟 클릭 후 페르소나별 필터 클릭 시 한반도 전체로 줌아웃 (2026-09-19, 대화 중 요청, 신규, 미착수)** — 특정 스팟을 클릭한 뒤 "페르소나별" 필터를 누르면 지도가 한반도 전체가 보일 정도로 크게 줌아웃됨. 5-3에서 파악한 "페르소나 bounds-fit이 GPS를 무시함" 문제와 같은 계열일 가능성 있음(전체 페르소나 스팟이 서울/부산/제주 등 전국에 흩어져 있으면 그 전부를 담는 bounds가 한반도 전체가 될 수 있음) — 다만 "특정 스팟 클릭 후"라는 선행 조건이 왜 필요한지는 미확인. 원인 조사 전, 미착수.
