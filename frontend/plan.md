...
- [ ] **지도 — 페르소나별 탭 검색 버그 (5-3)** (`"5-3\."` 검색) — 페르소나별 탭 적용 시 검색창 검색/"이 지역에서 검색"이 동작 안 함. 원인 조사 예정(아직 미수정).
...
- [ ] **5. 주변스팟 영역 처리 필요 (2026-09-19, 대화 중 요청, 신규)** — 우측 목록 패널에 섹션이 여러 개(검색결과/찜/이 지역 연관 관광지/주변 스팟) 켜질 수 있는데, 각 섹션이 차지하는 영역 때문에 정작 "주변 스팟"이 화면에 아예 안 보이는 문제. 찜/연관관광지 섹션 자체도 영역이 매우 협소함.
  ...
  - **5-3.** 타이틀 변경 부분 **(완료)** — 페르소나별(스타별) 탭에서 "주변 스팟" 대신 "페르소나 방문 장소"로 타이틀 이름 변경(`MapPage.tsx`의 `spotListTitle`). **버그는 미수정, 원인 조사 예정**: 페르소나별 탭 적용 시 검색창 검색 / "이 지역에서 검색" 버튼이 전부 동작하지 않는 문제 — 원인 파악 후 별도로 기록 예정(수정은 별도 확인 후).
