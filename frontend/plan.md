...
- [ ] **지도 — 주변 스팟 섹션별 노출 우선순위 (5-2)** (`"5-2\."` 검색) — 검색결과/찜/연관관광지가 다 켜지면 주변 스팟이 안 보이는 문제, 섹션별 우선순위 미정. 우선순위 추천 → 컨펌 후 개발 착수(아직 구현 금지). (5-1 SNS분석기 섹션 분리/5-3 타이틀 변경/5-4 검색결과 닫기 버튼은 완료됨.)
...
- [ ] **5. 주변스팟 영역 처리 필요 (2026-09-19, 대화 중 요청, 신규)** — 우측 목록 패널에 섹션이 여러 개(검색결과/찜/이 지역 연관 관광지/주변 스팟) 켜질 수 있는데, 각 섹션이 차지하는 영역 때문에 정작 "주변 스팟"이 화면에 아예 안 보이는 문제. 찜/연관관광지 섹션 자체도 영역이 매우 협소함.
  ...
  - **5-2. (미착수, 컨펌 대기)** 검색결과/찜/이 지역 연관 관광지가 다 켜지면 각 섹션이 차지하는 영역 때문에 주변 스팟이 전혀 안 보임(찜/연관관광지 영역도 매우 협소). 섹션별 우선순위를 정해야 함 — 5-1과 연계해서 "어떤 경우에 어떤 목록을 어떻게 보여줄지" 우선순위를 추천 → **컨펌 후 개발 시작(아직 구현 금지)**.
