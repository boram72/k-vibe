...
### 내루트
- [ ] **10. 같은 장소를 내 루트에 중복 추가할 수 있음** — 중복 추가를 막아야 함.
- [ ] **11. 로그인 상태에서 로그아웃해도 "내 루트"가 초기화되지 않음** — 로그아웃 시 초기화되도록 수정 검토.

### 제안사항
- [ ] **12. 루트 순서 변경 시 체감 변화가 없음(제안)** — 이동시간 표시, 최단거리/최소비용 등 내비게이션식 제안 기능 추가를 검토(우선순위 낮음, 제안 단계).
- ~~SNS 분석 화면에 사용 예시 GIF 추가(제안)~~ — 실제 GIF 콘텐츠 제작이 필요한 디자인/콘텐츠 작업이라 **이번 작업 범위에서 제외**("내가 수정할 부분 아님").
