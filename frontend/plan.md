...
- [ ] **1. 홈화면 수정**
  - [ ] 수원이가 그려준 샘플 반영
  - [ ] 프로필 좌측에 있던 "찜한 장소" 노출 영역을 홈 화면에 카드 형태로 이동
  - [ ] K-콘텐츠 페르소나 선택 UX 개선 — 쇼츠처럼 장소가 넘어가는 방식, 모바일에서 사진 노출 방식은 별도 확정 필요
  - [ ] 하단에 내 루트 미니맵 추가
...
- [ ] **4. 다른 페이지 보다가 돌아왔을 때 내 위치로 복귀하는 기능** (담당자 미정 — 보드에 "?")
...
- [ ] **12. 지도 스타별 필터 ↔ 페르소나 장소 실제 연동** (`FRONTEND_TODO_map_pan_search.md` 3번째 항목, 백엔드가 신규 엔드포인트 준비 완료) — `GET /personas/places?locale=`(페르소나 태그가 붙은 실제 장소, `Place` 타입과 1:1 대응)를 지도 진입 시 1회 불러와 `MapPage.tsx`의 `candidates`에 `id` 기준 중복 제거 병합. 기존 `matchStar`(place.tags 기반) 로직은 코드 변경 없이 그대로 재사용됨. **"전체" 클릭 시엔 페르소나 장소 전체를 위치 반경과 무관하게 노출**(카테고리 탭의 "전체"와 의미가 다름 — 대화로 확정), 각 장소 행에 소속 `persona.label`을 태그로 별도 표시(기존 카테고리 태그와 구분). 13번에서 만든 빨간 펄스 "내 위치" 마커(`route-mini-map.tsx`의 `CurrentLocationPin`)를 지도 메뉴(`map-canvas.tsx`)에도 동일하게 표기
...
- [ ] **14. 리뷰가 보였다 안 보였다 하는 버그 수정** (`FRONTEND_TODO_map_pan_search.md` 2번째 항목, 별도 브랜치/PR로 진행 중) — `api/reviews.ts`의 `withFallback()`이 API 실패 시 조용히 브라우저별 `localStorage` mock으로 새서 기기/시크릿창마다 다르게 보이던 문제. 실제 백엔드가 이미 정상 동작 중이므로(다른 `api/*.ts`처럼 "백엔드 아직 없음" 케이스가 아님) `withFallback` 제거하고 직접 호출, 실패 시 에러+재시도 UI 추가 예정
- [ ] **15. 홈 진입 시 `/personas`가 en→ko로 2번 호출되는 문제 수정** (`FRONTEND_TODO_map_pan_search.md` 4번째 항목, 별도 브랜치/PR로 진행 중) — `i18n.init()`의 `lng` 초기값을 URL 경로에서 첫 렌더 전에 동기적으로 읽어오도록 수정 예정
