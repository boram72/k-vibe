...
- [ ] **지도 — 주변 스팟 섹션별 노출 우선순위 (5-2)** (`"5-2\."` 검색) — **설계 확정, 구현 시작 대상**: 검색결과/찜/연관관광지/SNS분석기/주변 스팟(또는 페르소나 방문 장소)을 단일 `activeSection` 상태로 관리(최대 하나만 노출), 우선순위는 "방금 한 행동이 우선"(recency). 찜/연관관광지 토글 중복 클릭도 상호배타로 변경, SNS 분석기 섹션에도 검색결과와 동일한 닫기 버튼 추가(닫으면 주변 스팟 노출). (5-1/5-3 타이틀 변경/5-4는 이미 완료됨.)
...
- [ ] **5. 주변스팟 영역 처리 필요 (2026-09-19, 대화 중 요청, 신규)** — 우측 목록 패널에 섹션이 여러 개(검색결과/찜/이 지역 연관 관광지/주변 스팟) 켜질 수 있는데, 각 섹션이 차지하는 영역 때문에 정작 "주변 스팟"이 화면에 아예 안 보이는 문제. 찜/연관관광지 섹션 자체도 영역이 매우 협소함.
  ...
  - **5-2. (설계 확정, 대화로 컨펌됨 — 구현 시작 대상)** 검색결과/찜/이 지역 연관 관광지/SNS 분석기가 다 켜지면 각 섹션이 차지하는 영역 때문에 주변 스팟이 전혀 안 보임(찜/연관관광지 영역도 매우 협소) — Playwright로 재현: 3개 섹션 동시 노출 시 "Nearby Spots" 타이틀이 뷰포트(844px) 밖(y=953)까지 밀려나 완전히 안 보임.
    - **확정된 설계**: 검색결과/찜/연관관광지/SNS분석기/주변 스팟(또는 페르소나 방문 장소)를 독립된 boolean 여러 개로 각자 켜고 끄는 대신, **"지금 보여줄 화면 하나"를 가리키는 단일 상태**로 관리 — 예: `activeSection: 'saved' | 'attractions' | 'searchResults' | 'analyzer' | null`(`null`이면 기본값인 주변 스팟/페르소나 방문 장소). 항상 최대 하나의 섹션만 화면에 보여서 공간 경쟁 자체가 구조적으로 없어짐.
    - **우선순위 원칙 — "방금 한 행동이 우선"(recency)**: 찜 버튼을 누르면 `activeSection`이 `'saved'`로 전환되고 그 순간 다른 섹션은 전부 자동으로 닫힘. 그 상태에서 이어서 검색하면 `'searchResults'`로 전환(찜은 자동으로 닫힘). 검색결과를 닫으면(5-4에서 만든 X 버튼) `activeSection`이 `null`로 돌아가 기본(주변 스팟/페르소나 방문 장소)이 다시 보임.
    - **찜/연관관광지 토글 버튼도 상호배타로 변경**: 지금은 두 토글 버튼(하트/스파클) 각각 독립된 boolean이라 **동시에 둘 다 켜는 중복 클릭이 가능**한 상태 — 이것도 `activeSection` 하나로 흡수해서, 찜을 켠 채로 연관관광지 버튼을 누르면 찜은 자동으로 꺼지고 연관관광지만 켜지도록(중복 노출 자체가 불가능하게) 변경.
    - **SNS 분석기 섹션도 검색결과처럼 닫기 버튼 추가**: 5-4에서 검색결과에만 닫기(X) 버튼을 달았는데, SNS 분석기 섹션(5-1)에는 아직 없음 — 이번에 동일하게 닫기 버튼을 추가하고, 닫으면 `activeSection`이 `null`로 돌아가 주변 스팟이 노출되게 한다. (SNS 분석기 결과 자체, 즉 지도 핀이나 `focusPlaces` 데이터가 사라지는 게 아니라 그 섹션을 "지금 화면에서 보여주는 상태"만 닫는 것 — 검색결과 닫기와 동일한 성격.)
    - **"내 루트"에서 온 핸드오프는 별도 activeSection이 아님(대화로 확정)** — `MapFocusState.returnToRoute`(내 루트의 스팟 카드에서 지도 아이콘 클릭, 항상 `focusPlaces.length === 1`)로 들어온 경우는 SNS 분석기와 달리 "주변 스팟 관점에서 그 장소를 보는 것"에 가까워서 자기만의 섹션을 안 만듦 — 그냥 평소처럼 "주변 스팟" 목록에 포함시키되, 그 장소 행에만 태그(예: "내 루트")를 붙여서 구분되게 표시. 따라서 `activeSection` 초기값은 `focusState?.returnToRoute`일 땐 `'analyzer'`가 아니라 `null`(기본 주변 스팟)이어야 함 — 이미 존재하는 `routeOriginPlaceId`(현재 `PlaceDetailSheet`의 "루트로 돌아가기" 버튼에만 쓰임)를 목록 태그 표시에도 재사용.
    - **다음 단계**: 설계 컨펌 완료 — 구현 착수 가능.
