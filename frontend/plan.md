...
- [ ] **지도 — 줌 관련 버그 3건 (5-3 finding 1 / 6 / 7)** (`"원인 확인 완료, 수정 방향 컨펌됨"` 검색) — 전부 원인 확인 + 수정 방향 컨펌 완료, **구현 대기**: ① 특정 장소 선택해도 줌인이 안 됨(6번, `level` prop 고정값이라 bounds-fit 이후 재적용 안 됨 → focusCenter 세팅 시 `setLevel(4)` 명시 호출로 수정 예정) ② 페르소나별 진입/전환 시 GPS 무시하고 카탈로그 좌표만으로 bounds 계산해서 "확대"가 아니라 "점프"처럼 보임(5-3/7번, bounds 계산에 현재 카메라 위치도 포함시키는 방향으로 수정 예정 — 줌아웃 자체는 의도된 동작으로 유지, 대화 중 방향 재확인됨).
- [ ] **지도 — 페르소나별 "전체" 모드에서 카테고리별 주변 스팟 안 사라짐 (8, 신규)** (`"8\. 페르소나별"` 검색) — 원인 확인 완료, 수정 방향 컨펌됨, **구현 대기**: `matchStar`의 "전체" 분기가 "태그 유무"만 보고 페르소나 라벨인지는 확인 안 해서, 일반 카테고리 장소(설명용 태그 보유)도 페르소나 장소로 착각해 계속 지도에 남음.
...
- [ ] **5. 주변스팟 영역 처리 필요 (2026-09-19, 대화 중 요청, 신규)** — 우측 목록 패널에 섹션이 여러 개(검색결과/찜/이 지역 연관 관광지/주변 스팟) 켜질 수 있는데, 각 섹션이 차지하는 영역 때문에 정작 "주변 스팟"이 화면에 아예 안 보이는 문제. 찜/연관관광지 섹션 자체도 영역이 매우 협소함.
  ...
  - **5-3.** 타이틀 변경 부분 **(완료)** — 페르소나별(스타별) 탭에서 "주변 스팟" 대신 "페르소나 방문 장소"로 타이틀 이름 변경(`MapPage.tsx`의 `spotListTitle`).
    - **버그 원인 조사 완료(수정 안 함, 대화 중 요청)** — 대화로 지적받은 2개 증상을 Playwright로 재현·검증:
      ...
      - **1번(줌 점프)은 아래 6/7번과 함께 수정 방향 확정(컨펌 완료, 구현 대기)** — 상세는 6/7번 항목 참고. **2번(위치 무관 고정 카탈로그)은 이번 라운드에서 수정 안 함** — 필요 시 향후 페르소나별 탭의 목록도 위치 기반 `/places` 결과와 결합하는 방향으로 재설계 검토.
...
- [ ] **6. 모바일 — 이 지역 연관 관광지 등 특정 스팟 검색/클릭 시 줌인 안 됨 (2026-09-19, 대화 중 요청) — 원인 확인 완료, 수정 방향 컨펌됨, 구현 대기**
  ...
  - **수정 방향(컨펌 완료)**: `focusCenter`가 세팅될 때(장소 하나를 포커스할 때) `map.setLevel(4)`를 명시적으로 호출하는 `useEffect` 추가 — bounds-fit이 줌을 아무리 넓혀놨어도, "장소 하나 선택"은 항상 확실하게 레벨 4로 줌인되도록 통일.
  - **예상 동작**: 목록 클릭/핀 클릭/검색결과 클릭/관광지 추천 클릭 등 무엇으로 들어왔든, 특정 장소 하나를 선택하면 지도가 그 위치로 이동하면서 항상 가까이 줌인됨.
- [ ] **7. 모바일 — 특정 스팟 클릭 후 페르소나별 필터 클릭 시 한반도 전체로 줌아웃 (2026-09-19, 대화 중 요청) — 원인 확인 완료, 수정 방향 컨펌됨(대화 중 방향 수정됨), 구현 대기**
