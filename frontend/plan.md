...
- [ ] **지도 — 페르소나별 "전체" 모드에서 카테고리별 주변 스팟 안 사라짐 (8)** (`"8\. 페르소나별"` 검색) — 원인 확인 완료, 수정 방향 컨펌됨, **구현 대기**: `matchStar`의 "전체" 분기가 "태그 유무"만 보고 페르소나 라벨인지는 확인 안 해서, 일반 카테고리 장소(설명용 태그 보유)도 페르소나 장소로 착각해 계속 지도에 남음.
- [ ] **지도 — 페르소나 탭에서 "이 지역에서 검색" 시 카테고리별 자동전환 안 됨 (9)** (`"9\. 페르소나 탭"` 검색) — 원인/범위 확정, 수정 방향 컨펌됨, 구현 대기.
...
- [ ] **8. 페르소나별 "전체" 모드에서 카테고리별 주변 스팟이 안 사라짐 (2026-09-19, 대화 중 발견) — 원인 확인 완료, 수정 방향 컨펌됨, 구현 대기**
  ...
  - **수정 방향(컨펌 완료)**: `matchStar`의 "전체" 분기를 `place.tags?.length > 0` 대신 **실제 페르소나 라벨과 일치하는 태그인지**로 좁힘 — `personaLabels = new Set(personaPlaces.flatMap(p => p.tags ?? []))`를 이미 불러와져 있는 `personaPlaces`에서 뽑아 `place.tags?.some((tag) => personaLabels.has(tag))`로 교체(API 재호출 없음).
  - **예상 동작**: 페르소나별 → "전체" 클릭 시 실제 페르소나 라벨(IU, BTS V 등) 태그를 가진 장소만 지도/목록에 남고, 카테고리성 일반 장소(경복궁 등)는 지도에서 사라짐. **API 재호출 없이 메모리에 있는 데이터로 필터 조건만 다르게 적용**하는 것이므로, 카테고리별로 돌아오면 그 즉시 다시 보임(별도 삭제/재조회 없음). 특정 스타(예: IU) 선택 상태는 원래도 라벨로 정확히 매칭되고 있어 이번 수정으로 동작 안 바뀜 — "전체" 분기만 고쳐짐.

- [ ] **9. 페르소나 탭에서 "이 지역에서 검색" 시 카테고리별로 자동 전환 안 됨 (2026-09-19, 대화 중 발견) — 원인/범위 확정, 구현 대기**
  ...
  - **수정 방향(컨펌 완료)**: `MapPage.tsx`의 `onSearchArea` 콜백 안에서 `setSearchCenter`/`setQueryCenter`와 함께 `setFilterMode('category')`도 호출.
  - **예상 동작**: 페르소나별 탭에서 지도를 드래그해 "이 지역에서 검색"을 확정하면, 탭 자체가 "카테고리별"로 전환되며 그 지역의 실제 주변 검색 결과가 정상적으로 보임. `starFilter`(예: IU 선택 상태)는 안 지워지고 기억되어, 나중에 페르소나별로 다시 돌아가면 그대로 복원됨(기존 "탭 전환 시 서로 선택값 유지" 원칙과 동일).
