...
- [ ] **7. 도슨트 기능 삭제** (`docent-player.tsx` 및 연동부)
- [ ] **8. 카카오 로그인 삭제** — Google만 남기는 방향(2026-09 OAuth 실연동 작업(PR #35)과 배치되는 결정이라 착수 전 재확인 필요)
- [ ] **9. K-Vibe 지도에 '스타별'/'카테고리별' 필터 추가** (DB 태그: 가현 담당)
- [ ] **10. 우측 상단 '계정' 클릭 시 내정보/설정만 보이도록** (현재 노출되는 장소 목록 제거)
- [ ] **11. 하트 버튼 라벨 검토** — "저장장소"를 "찜"으로 바꾸는 안 검토(내 루트 저장과 헷갈릴 수 있어 제안됨, "찜"이 더 직관적이라는 의견)
