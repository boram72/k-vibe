...
- [ ] **1. 홈화면 수정** (수원 샘플 이미지 반영, 대화로 세부사항 확정 — 2026-09)
  - [ ] **배너**: 그라데이션 배경(다크/라이트 각각 어울리는 톤 적용) + 인사말 + "내 루트 만들기" CTA(클릭 시 `/route`로 이동) + 우측 "OO 루트 미리보기" — 추천 페르소나 1명을 **무작위로** 뽑아서 보여주고, 홈 화면 진입할 때마다(마운트마다) 다시 무작위로 바뀜(로그인/게스트 구분 없음)
  - [ ] **"찜한 장소" 카드**(기존 "저장한 루트" 자리를 대체, 제목도 "찜한 장소"로) — `saved-places-grid.tsx`(10번 작업 때 프로필에서 빼면서 재사용 대비해둔 컴포넌트)를 홈으로 이동. 장소별 "내 루트 추가" 버튼 — 클릭 시 `addStopToRouteDraft()`로 추가 + `/route`로 이동, 이미 루트에 추가된 장소면 버튼 숨김. "이어서 편집" 문구/버튼은 없음(수원 샘플에는 있었으나 제외 확정)
  - [ ] **"페르소나로 루트 찾기"**: 상위 4개만 카드로 노출 — `routeCnt` 내림차순 정렬(이미 응답에 포함된 필드라 추가 계산/호출 없음), 값이 같으면 응답 순서 유지. 카드 호버 시 해당 persona가 방문한 장소 사진이 빠르게 전환(슬라이드쇼) — `GET /personas/places?locale=`(아래 4번 항목에서 프론트에 붙이는 것과 동일 엔드포인트, `label` 기준으로 그룹핑해 이미지 재사용)
  - [ ] 백엔드 응답이 느릴 때 배너/찜한장소/페르소나 카드 전부 스켈레톤 노출(페르소나 카드는 기존에도 있었음, 나머지 두 곳 신규)
...
- [ ] **12. 지도 스타별 필터 ↔ 페르소나 장소 실제 연동** (`FRONTEND_TODO_map_pan_search.md` 3번째 항목, 백엔드가 신규 엔드포인트 준비 완료) — `GET /personas/places?locale=`(페르소나 태그가 붙은 실제 장소, `Place` 타입과 1:1 대응)를 지도 진입 시 1회 불러와 `MapPage.tsx`의 `candidates`에 `id` 기준 중복 제거 병합. 기존 `matchStar`(place.tags 기반) 로직은 코드 변경 없이 그대로 재사용됨. **"전체" 클릭 시엔 페르소나 장소 전체를 위치 반경과 무관하게 노출**(카테고리 탭의 "전체"와 의미가 다름 — 대화로 확정), 각 장소 행에 소속 `persona.label`을 태그로 별도 표시(기존 카테고리 태그와 구분)
- [ ] **13. 내 루트 '현재 거리' 확인 문구 개선** — 기능 자체는 정상 동작(브라우저 GPS로 다음 스팟까지 haversine 거리 계산, 백엔드 호출 없음). 다만 도착지만 나오고 출발 기준점이 안 보여서 "현재 위치에서 {{name}}까지 {{distance}}"처럼 시작~끝 둘 다 명시하도록 문구만 수정(`route.next_stop_near`/`next_stop_far`)
