...
- [x] **8. 서로 다른 진입점(페르소나/지도)으로 같은 장소를 추가하면 중복됨** — QA 10번 중복방지(`placeId` 비교)가 같은 출처끼리는 걸러내지만, 페르소나로 추가한 장소를 나중에 지도에서 다시 검색해 추가하면 걸러지지 않고 중복 추가됨.
  ...
  - **수정**: `isSamePlace()`에 보조 판정 추가 — id/placeId가 달라도 이름(공백 제거+소문자 정규화 후 완전일치)이 같고 좌표가 50m(`SAME_PLACE_DISTANCE_KM`) 이내면 같은 장소로 간주해 걸러냄. `haversine.ts`의 기존 `haversineKm()` 재사용. `addStopToRouteDraft`/`addStopsToRouteDraft`/`restoreRouteDraftFromServer` 전부 이 함수 하나만 공유해서 쓰므로 수정 지점은 한 곳. Playwright로 지도 스타일 id("126765")로 추가한 "경복궁"과, 좌표가 ~4m만 다르고 페르소나 스타일 id("BTS뷔-경복궁")인 "경복궁"을 연달아 추가 → 두 번째는 걸러짐(added: false) 확인, 반대로 이름이 다른 장소("스타벅스 강남점")는 정상 추가되는 것도 함께 확인(false positive 없음).
...
- [ ] **10. 모바일 홈 화면 — 페르소나 카드 프로필 이미지 롱프레스 시 슬라이드쇼 팝업과 "사진 저장하기" 네이티브 메뉴가 동시에 뜸** — **진단 완료, 미수정**(대화 중 요청대로 8번 먼저 진행). `persona-picker.tsx`의 `PersonaCard`는 이미 `onContextMenu={(e) => e.preventDefault()}`를 카드 버튼에 걸어뒀지만, 이건 DOM `contextmenu` 이벤트(주로 데스크탑 우클릭, 일부 브라우저의 롱프레스 말기에 발생)만 막을 뿐, **iOS Safari가 `<img>` 태그 자체에 발동시키는 네이티브 "터치 콜아웃"(Save Photo/Copy/Share 액션시트)은 완전히 다른 메커니즘**이라 안 막힘 — 이 콜아웃은 `-webkit-touch-callout` CSS 속성으로만 끌 수 있는데, `index.css`/`persona-picker.tsx`/`zoomable-image.tsx` 어디에도 이 속성이 설정되어 있지 않음(grep으로 확인, 코드베이스 전체에 0건). 즉 커스텀 롱프레스 타이머(350ms)와 iOS/Android 브라우저 자체의 롱프레스 이미지 제스처가 같은 물리적 터치를 각자 독립적으로 감지해서 팝업+네이티브 메뉴가 동시에 뜨는 것. **수정 방향(미구현)**: 카드 안의 `<img>`(및 롱프레스로 뜨는 85% 팝업 이미지)에 `style={{ WebkitTouchCallout: 'none' }}`(+ 안전판으로 `-webkit-user-select: none`/`user-drag: none`) 추가.
