...
상태관리 플로우 : https://claude.ai/code/artifact/5fa9cda4-6b7d-40e7-afec-b83f1f2cc2ca
...
- [x] **메모 — DB 이전 대상 확정** (2026-07-19 결정, Step15 ②/④ 범위에 포함): localStorage 직접 호출하는 lib 5개(`route-draft.ts`/`route-progress.ts`/`persona-preference.ts`/`theme-store`/`sidebar-store`) 중 **`theme-store`/`sidebar-store`는 기기별 UI 상태라 DB 이전 불필요, 로컬 유지 확정**. 나머지 3개는 DB 이전 대상으로 확정:
  - `persona-preference.ts`(`k-vibe-persona-preference`) — 홈피드 개인화 힌트(마지막 선택 theme/detail). 이전 시 로그인 유저 간 기기 이동해도 개인화 유지됨
  - `route-progress.ts`(`k-vibe-route-progress`) — 완료 스팟 ID Set. RoutePage/ProfilePage 공유 상태라 이전 가치 높음
  - `route-draft.ts`(`k-vibe-current-route`/`k-vibe-persona-plan`) — 실제 루트 데이터(스팟 목록). 이전 가치 가장 높은 후보
  - 백엔드 `data_repositories/routeinfo.py`(`userroute` 테이블, `GET/PUT /route/{username}`)가 이미 유사 개념 존재하나 `location: str` 1개 필드뿐이라 프론트의 `RouteStop`(좌표/카테고리/체류시간/완료여부 등 포함) 스펙과 맞지 않음 — 스키마 확장 또는 신규 테이블 필요. `data_repositories/personainfo.py`(`persona` 테이블)는 "K팝 스타 경로" 도메인이라 프론트의 persona-preference(theme/detail 위저드 선호도)와 이름만 같고 무관한 별개 개념 — 혼동 주의
...
- [x] **로그인 방식 확정**: OAuth만(ID/PW 폼 없음), provider는 **Google+Apple+Kakao** 3개(Facebook/GitHub/Naver/Instagram 제외 — Naver/Instagram은 Supabase 자체가 미지원이라는 걸 조사로 확인). 프론트 제1원칙("서버 교체 시 API 정책만 바꾸면 그대로 동작") 재확인 → `@supabase/supabase-js` 설치 안 함, 전부 mock으로 구현
  - **2026-07-20 수정— Apple 제외**: Apple Developer가 유료 멤버십(Services ID+private key 발급에 필요) 요구라 실제 연동 단계에서 제외 결정. `AuthProvider` 타입/`MOCK_USERS`/`PROVIDER_BUTTONS`/i18n `login.continue_apple` 키 전부 제거, **Google+Kakao 2개로 축소**
...
- [ ] **③ Supabase Auth (OAuth: Google/Kakao)** — `src/lib/auth.ts` 내부만 교체 예정(호출부 무변경). Apple은 유료 멤버십 필요해 2026-07-20 제외 확정
  - [ ] Supabase 프로젝트 생성, provider별 콘솔 설정(Google Cloud Console / Kakao Developers)
  - [ ] `getCurrentUser()`/`loginWithProvider()`/`logout()` 내부 구현 교체
- [ ] **④ Supabase DB — localStorage → DB 이전 대상** (③에 종속, 각 lib 파일 내부만 교체 예정·호출부 무변경). 2026-07-19 대상 확정, 상세 판단 근거는 Step10 후속2 메모 참고:

  | lib 파일 | localStorage 키 | 이전 대상 | 전송 시점 |
  |---|---|---|---|
  | `lib/auth.ts` | `k-vibe-mock-session` | Supabase Auth (`auth.users`) | 로그인/회원가입 액션 시 즉시 (이미 실구현됨) |
  | `lib/saved-places.ts` | `k-vibe-saved-places:{userId\|guest}` | Supabase DB `saved_places` 테이블(신규 설계 필요) | **즉시 전송** — 하트 클릭은 단발 이산 이벤트, 디바운스 걸면 오히려 UX 저하(찜했는데 반영 지연) |
  | `lib/route-progress.ts` | `k-vibe-route-progress` | Supabase DB 신규 테이블 또는 기존 테이블 컬럼 확장(신규 설계 필요) — 우선순위 중 | **즉시 전송** — 완료 체크도 사람 클릭 단위라 스팸 우려 없음 |
  | `lib/persona-preference.ts` | `k-vibe-persona-preference` | Supabase DB 신규 테이블(신규 설계 필요) — 우선순위 하 | **즉시 전송** — 위저드 완료 시 1회뿐, 배칭 불필요 |
  | `lib/route-draft.ts` | `k-vibe-current-route`, `k-vibe-persona-plan` | Supabase DB 신규 테이블(신규 설계 필요) — 우선순위 최상 | **디바운스 전송** — 드래그 재정렬 등 연속 편집 발생하므로 마지막 변경 후 1~2초 뒤 1회 PUT(전체 덮어쓰기, `routeinfo.py`의 delete→재삽입과 동일 패턴) + 페이지 이탈/탭 숨김 시 강제 flush 안전망 |

  전송 빈도 기준: `route-draft`만 연속 편집이 실제로 발생하는 유일한 케이스라 디바운스가 필요하고, 나머지 4개(auth/saved-places/route-progress/persona-preference)는 전부 "사람이 클릭하는 순간에만" 값이 바뀌는 단발 이벤트라 즉시 전송이 표준.

  - [ ] `saved_places` 테이블 스키마 설계(`user_id`/장소 정보 저장 방식 결정) + `fetchSavedPlaces()`/`toggleSavedPlace()` 내부 구현 교체
  - [ ] route-draft/route-progress/persona-preference 3개 테이블 스키마 설계. 백엔드 `data_repositories/routeinfo.py`의 `userroute` 테이블(`GET/PUT /route/{username}`)이 route-draft와 개념 유사하나 컬럼이 `id`/`username`/`order`/`location`(문자열)뿐이라 프론트 `RouteStop`(좌표/카테고리/체류시간/시작시각/완료여부/`fromPersona` 등) 스펙엔 부족 — 확장 또는 신규 테이블 필요. `data_repositories/personainfo.py`의 `persona` 테이블은 "K팝 스타별 고정 경로" 도메인이라 persona-preference(테마/디테일 위저드 선호도)와 이름만 같고 무관 — 재사용 불가
  - [ ] `theme-store`/`sidebar-store`는 기기별 UI 상태라 **DB 이전 대상에서 제외 확정** (로컬 유지)
  - [ ] **참고**: `backend/db/schema.sql`에 `user`/`location`/`persona`/`docent`/`userroute` DDL이 있지만 이건 "설계도" 파일일 뿐 자동 반영 안 됨(Supabase SQL Editor에 수동 실행 필요). 실제 연결된 Supabase 프로젝트에 `user` 테이블조차 아직 없는 것으로 확인됨(`/user/login` 호출 시 `Could not find the table 'public.user'` 에러) — 백엔드 작업 전 schema.sql 실행 여부부터 확인 필요
  - [ ] **DB 연동 후 실제 테스트해야 할 known limitation 3건** (2026-07-20 db-sync.ts 준비 작업 중 리뷰로 발견, 지금은 백엔드가 없어 재현 불가 — 실제 엔드포인트 붙인 뒤 반드시 확인):
    1. **`beforeunload` flush 신뢰성**: `app-layout.tsx`의 `flushRouteDraftSync()`가 일반 axios PUT이라 `beforeunload` 핸들러 안에서 브라우저가 완료를 보장 안 함(요청이 페이지 종료 전에 취소될 수 있음). `visibilitychange`("hidden")가 대부분의 실제 탭종료/새로고침 상황에서 먼저 발동해 어느 정도 커버는 되지만, `beforeunload` 백업 경로 자체는 `navigator.sendBeacon()` 또는 `fetch(url, {keepalive:true})`로 바꿔야 확실함
    2. **동시 즉시전송 요청 순서 꼬임**: `route-progress`(완료 체크)/`saved-places`(하트)를 빠르게 연타하면 `syncImmediately()`가 매번 별도 PUT을 쏘는데 순서 보장이 없어, 네트워크 상황에 따라 늦게 도착한 이전 요청이 최신 요청을 덮어써서 DB가 로컬 상태와 어긋날 수 있음. 클라이언트 사이드 요청 시퀀싱(예: 매 요청에 타임스탬프 포함해 백엔드가 stale write 거부) 또는 이 둘도 짧게 디바운스하는 방안 검토 필요
    3. **RoutePage 마운트 시 불필요한 sync 예약**: `RoutePage.tsx`의 `useEffect(() => saveRouteDraft(stops), [stops])`가 마운트 시에도 실행돼, 아무것도 안 바꾸고 그냥 열기만 해도 디바운스 sync가 예약됨. 데이터가 동일해 결과적으로 틀린 값이 저장되진 않지만 불필요한 네트워크 호출 — 경미하나 트래픽 절감 차원에서 손볼 만함
